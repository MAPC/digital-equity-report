...
const Acknowledgements = () => {
  ...
  function createList(data) {
    return data.map((li, index) => <li key={index}>{li}</li>);
  }
  ...
  return (
    <div>
      <div className="section">
        ...
        <p>
          To achieve outcomes that enhance the wellbeing of the community, it’s
          essential that community perspective guides the work. A community
          advisory team served as a core working group which meet throughout the
          process to provide feedback, responses to proposed strategies and data
          analysis. This group was essential in assisting with the distribution
          and completion of the community needs assessment. The perspective
          provided by the community advisory helped inform the development of
          this plans vision and goals, and shaped the focus and framing of this
          report. Ultimately, it will be this set of stakeholders will be able
          to take actionable measures towards the goals this report intends to
          achieve.
        </p>
      </div>
      <div className="section section-two-col-flex">
        <div className="section-half border-right">
          ...
          <ul>{createList(everett)}</ul>
          <h5>City of Chelsea</h5>
          <ul>{createList(chelsea)}</ul>
          <h5>City of Revere</h5>
          <ul>{createList(revere)}</ul>
        </div>
        <div className="section-half">
          <h5>Community Organizations</h5>
          <ul>{createList(community)}</ul>
          <h5>MAPC Staff</h5>
          <ul>{createList(mapc)}</ul>
        </div>
      </div>
    </div>
  );
