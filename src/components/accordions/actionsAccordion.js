...
export default function ActionsAccordion() {
  ...
  const handleChangeImmediate = (panel) => (event, newExpanded) => {
    ...
    const accordionTitles = event.target.closest(
      `.acc-action-immediate`
    ).children;
    const offset =
      event.target.closest(`.acc-action-immediate`).getBoundingClientRect()
        .top + window.scrollY;
  ...
  const handleChangeIntermediate = (panel) => (event, newExpanded) => {
    ...
    const offset =
      event.target.closest(`.acc-action-intermediate`).getBoundingClientRect()
        .top + window.scrollY;
    const pnlIdx = parseInt(panel.substring(panel.length - 1));
  ...
  const handleChangeLongTerm = (panel) => (event, newExpanded) => {
    ...
    const offset =
      event.target.closest(`.acc-action-longterm`).getBoundingClientRect().top +
      window.scrollY;
    const pnlIdx = parseInt(panel.substring(panel.length - 1));
    let titleOffset = pnlIdx * 80;
  ...
  const longterm = [
    {
      ...
      description: [
        <span>
          Building on the infrastructure investments that have already been made
          by Everett, Chelsea, and Revere to connect municipal buildings and
          assets, there is an opportunity to align future investments and
          maintenance. The Division of Local Services has established a{" "}
          <a href="https://www.mass.gov/municipal-fiber-grant-program">
            Municipal Fiber Grant Program
          </a>{" "}
          that opened for the first time in March of 2020. Structured like other
          competitive Community Compact grants, this program allows more funds
          to go towards multi-jurisdictional projects up to a maximum of
          $500,000 may be awarded to a project. This is an opportunity to
          address deficiencies in municipal networks identified in reports (like
          Chelsea{"'"}s{" "}
          <a href="https://mapc365.sharepoint.com/:f:/s/DigitalAccess/ErM3RPx1sz5BocIKT599V7kBHXnduTFtiquhvRDDpJ-iVg?e=KdN7uP">
            Dewsbury report
          </a>
          ) while establishing robust fiber option networking to support network
          monitoring, cyber security, records management, and backup and
          recovery. Cohesive and collaborative inter-municipal network
          connection also creates opportunities to gain economies of scale by
          aggregating internet bandwidth purchases and the associated security
          infrastructure.{" "}
        </span>,
      ],
      steps: [
  ...
  function createAccordions(actions, type, changeFunction, expandSetting) {
    return actions.map((action, index) => {
      return (
        <AnchorLink to={`#${type}-panel${index}`} key={index}>
          <Accordion
            className="actionAccordion"
            id={`${type}-panel${index}`}
            expanded={expandSetting === `${type}-panel${index}`}
            onChange={changeFunction(`${type}-panel${index}`)}
            onLoad={changeFunction(`${type}-panel${index}`)}
          >
            <AccordionSummary
