...
const ReportMenu = ({ isActive, setActive, setTabIndex, setAssessIndex }) => {
    ...
    if (element.type === "main") {
      ...
      return (
        <AnchorLink
          ...
          to={`#` + element.link}
          onAnchorLinkClick={() =>
            handleTabsChange(element.tab, element.callback)
          }
          className={
            isActive !== element.parent
              ? "report-menu__sublink hidden"
              : "report-menu__sublink"
          }
        >
          {element.title}
        </AnchorLink>
  ...
  return <div className="report-menu">{menuLinks}</div>;
};
